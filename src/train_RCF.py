...
import matplotlib
matplotlib.use('Agg')
from src.data_loader import BSDS_RCFLoader
from src.models import RCF
from src.functions import cross_entropy_loss_RCF
from torch.utils.data import DataLoader
from src.utils import Logger, Averagvalue, save_checkpoint, load_vgg16pretrain, get_yaml_config
...
# ================ datasets
parser.add_argument('--dataset_name', help='dataset_used', default='HED-BSDS', metavar='DATA')
parser.add_argument('--loader_workers', help='num workers', default=8, type=int, metavar='DATA')

# Set folders
THIS_DIR = abspath(dirname(__file__))
ROOT_DIR = abspath(dirname(THIS_DIR))
TMP_DIR = join(ROOT_DIR, "tmp", "RCF")
MDL_DIR = join(ROOT_DIR, "model")
if not isdir(TMP_DIR):
    os.makedirs(TMP_DIR)

# Get/Set  configs
data_config = get_yaml_config(join(ROOT_DIR, "config", "datasets.yaml"))
...
#dataset_set
try:
    dataset_conf = data_config[args.dataset_name]
except yaml.YAMLError:
    print(f"{args.dataset_name} config not implemented, exiting..")
    sys.exit(1)
...
def main():
    args.cuda = True
    # datasets
    train_dataset = BSDS_RCFLoader(config=dataset_conf, split="train")
    test_dataset = BSDS_RCFLoader(config=dataset_conf, split="test")
    train_loader = DataLoader(
        train_dataset, batch_size=args.batch_size,
        num_workers=args.loader_workers, drop_last=True, shuffle=True)
    test_loader = DataLoader(
        test_dataset, batch_size=args.batch_size,
        num_workers=args.loader_workers, drop_last=True, shuffle=False)

    with open(test_dataset.filelist_path, 'r') as f:
    ...
    load_vgg16pretrain(model, vggmodel=join(MDL_DIR, "vgg16convs.mat"))
